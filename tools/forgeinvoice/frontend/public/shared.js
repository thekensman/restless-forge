/* shared.js — ForgeInvoice shared header & footer
   Single source of truth for ALL ForgeInvoice pages (main app + sub-pages).
   Requires /shared.js (site/shared.js) to be loaded first for rf* utilities.

   Placeholders (replace across the whole template):
     forgeinvoice   — URL directory name,  e.g. "my-tool"
     ForgeInvoice  — Display name,        e.g. "My Tool"
     fi — Identifier prefix,   e.g. "mt"  (2–5 lowercase letters)
...
(function () {
  ...
  window.fiHeader = function () {
    ...
    return '<header class="site-header"><div class="site-header__inner">' +
      ...
      '<a class="site-header__brand" href="' + base + '/">🧾 ForgeInvoice</a>' +
  ...
  window.fiFooter = function () {
    ...
    return '<footer class="footer">' +
      ...
      '<p class="footer__copy">&copy; 2026 <a href="/" style="color:inherit;text-decoration:none;">Restless Forge</a> &mdash; ForgeInvoice.</p>' +
      '</footer>';
  };

  document.addEventListener('DOMContentLoaded', function () {
    var headerEl = document.getElementById('fi-header');
    if (headerEl) headerEl.outerHTML = window.fiHeader();
    var footerEl = document.getElementById('fi-footer');
    if (footerEl) footerEl.outerHTML = window.fiFooter();
  });
})();
