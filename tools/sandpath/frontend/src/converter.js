...
function parsePath(d) {
    ...
    let lc = null;
    const nf = () => { i++; return parseFloat(tokens[i]); };
    const more = () => i + 1 < tokens.length && !/[MmLlHhVvCcSsQqTtAaZz]/.test(tokens[i + 1]);
...
function arcToLines(cx, cy, _rx, _ry, _rot, _la, _sw, ex, ey, n = 24) {
    const segs = [];
    let px = cx, py = cy;
...
function sampleSubpath(subpath, n) {
    const all = [];
    ...
    function walk(el, ptx) {
        ...
        if (tag === "path") {
            ...
            if (coords.length >= 4) {
                ...
                for (let j = 2; j < coords.length - 1; j += 2)
                    d += ` L ${coords[j]},${coords[j + 1]}`;
                if (tag === "polygon")
                    d += " Z";
                results.push({ d, tx: ct[0], ty: ct[1], sx: ct[2], sy: ct[3] });
            }
...
// ─── Public API ───────────────────────────────────────────────
export function convert(svgText, device, opts = {}) {
