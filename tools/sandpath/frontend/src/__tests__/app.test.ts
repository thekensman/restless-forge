...
describe("File type detection", () => {
  ...
  function detectFileMode(name: string, type: string): "svg" | "image" {
    const ext = "." + name.split(".").pop()?.toLowerCase();
    if (SVG_EXTS.has(ext) || type === "image/svg+xml") return "svg";
...
describe("File validation", () => {
  ...
  function validateFile(name: string, size: number): string | null {
    const ext = "." + name.split(".").pop()?.toLowerCase();
    if (!VALID_EXTS.has(ext)) return "Unsupported file type";
    if (size > MAX_SIZE) return "File too large";
    return null;
  }
...
describe("Stats grid rendering", () => {
  function buildStatsHtml(stats: Record<string, any>): string {
    ...
    if (stats.subpaths != null) entries.push(["Subpaths", String(stats.subpaths)]);
    if (stats.fit) entries.push(["Fit mode", stats.fit]);
    return entries.map(([l, v]) =>
      `<div class="stat"><div class="stat__label">${l}</div><div class="stat__value">${v}</div></div>`
    ).join("");
  }
...
describe("HTML structure", () => {
  ...
  it("trace section starts hidden", () => {
    const sec = document.getElementById("trace-section") as HTMLElement;
    expect(sec.style.display).toBe("none");
  });

  it("results section starts hidden", () => {
    const sec = document.getElementById("results-section") as HTMLElement;
    expect(sec.style.display).toBe("none");
  });

  it("error toast starts hidden", () => {
    const el = document.getElementById("error-toast") as HTMLElement;
...
describe("Error toast", () => {
  ...
  function showError(msg: string) {
    ...
    const msgEl = document.getElementById("error-msg") as HTMLParagraphElement;
