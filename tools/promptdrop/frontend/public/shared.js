/* shared.js — PromptDrop shared header & footer
   Single source of truth for ALL PromptDrop pages (main app + sub-pages).
   Requires /shared.js (site/shared.js) to be loaded first for rf* utilities.

   Placeholders (replace across the whole template):
     promptdrop   — URL directory name,  e.g. "my-tool"
     PromptDrop  — Display name,        e.g. "My Tool"
     pdr — Identifier prefix,   e.g. "mt"  (2–5 lowercase letters)
   ...
   these two empty divs; this script auto-injects on DOMContentLoaded:
     <div id="pdr-header"></div>
     <div id="pdr-footer"></div>
*/
(function () {
  'use strict';
  ...
  window.pdrHeader = function () {
    ...
    return '<header class="site-header"><div class="site-header__inner">' +
      ...
      '<a class="site-header__brand" href="' + base + '/">💧 PromptDrop</a>' +
  ...
  window.pdrFooter = function () {
    ...
    return '<footer class="footer">' +
      ...
      '<p class="footer__copy">&copy; 2026 <a href="/" style="color:inherit;text-decoration:none;">Restless Forge</a> &mdash; PromptDrop.</p>' +
      '</footer>';
  };

  document.addEventListener('DOMContentLoaded', function () {
    var headerEl = document.getElementById('pdr-header');
    if (headerEl) headerEl.outerHTML = window.pdrHeader();
    var footerEl = document.getElementById('pdr-footer');
    if (footerEl) footerEl.outerHTML = window.pdrFooter();
  });
})();
