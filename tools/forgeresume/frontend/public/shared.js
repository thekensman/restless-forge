/* shared.js — ForgeResume shared header & footer
   Single source of truth for ALL ForgeResume pages (main app + sub-pages).
   Requires /shared.js (site/shared.js) to be loaded first for rf* utilities.

   Placeholders (replace across the whole template):
     forgeresume   — URL directory name,  e.g. "my-tool"
     ForgeResume  — Display name,        e.g. "My Tool"
     fr — Identifier prefix,   e.g. "mt"  (2–5 lowercase letters)
   ...
   these two empty divs; this script auto-injects on DOMContentLoaded:
     <div id="fr-header"></div>
     <div id="fr-footer"></div>
*/
(function () {
  'use strict';
  ...
  window.frHeader = function () {
    ...
    return '<header class="site-header"><div class="site-header__inner">' +
      ...
      '<a class="site-header__brand" href="' + base + '/">📝 ForgeResume</a>' +
  ...
  window.frFooter = function () {
    ...
    return '<footer class="footer">' +
      ...
      '<p class="footer__copy">&copy; 2026 <a href="/" style="color:inherit;text-decoration:none;">Restless Forge</a> &mdash; ForgeResume.</p>' +
      '</footer>';
  };

  document.addEventListener('DOMContentLoaded', function () {
    var headerEl = document.getElementById('fr-header');
    if (headerEl) headerEl.outerHTML = window.frHeader();
    var footerEl = document.getElementById('fr-footer');
    if (footerEl) footerEl.outerHTML = window.frFooter();
  });
})();
