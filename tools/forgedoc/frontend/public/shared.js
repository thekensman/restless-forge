/* shared.js — ForgeDoc shared header & footer
   Single source of truth for ALL ForgeDoc pages (main app + sub-pages).
   Requires /shared.js (site/shared.js) to be loaded first for rf* utilities.

   Placeholders (replace across the whole template):
     forgedoc   — URL directory name,  e.g. "my-tool"
     ForgeDoc  — Display name,        e.g. "My Tool"
     fd — Identifier prefix,   e.g. "mt"  (2–5 lowercase letters)
...
(function () {
  ...
  window.fdHeader = function () {
    ...
    return '<header class="site-header"><div class="site-header__inner">' +
      ...
      '<a class="site-header__brand" href="' + base + '/">📄 ForgeDoc</a>' +
  ...
  window.fdFooter = function () {
    ...
    return '<footer class="footer">' +
      ...
      '<p class="footer__copy">&copy; 2026 <a href="/" style="color:inherit;text-decoration:none;">Restless Forge</a> &mdash; ForgeDoc.</p>' +
      '</footer>';
  };

  document.addEventListener('DOMContentLoaded', function () {
    var headerEl = document.getElementById('fd-header');
    if (headerEl) headerEl.outerHTML = window.fdHeader();
    var footerEl = document.getElementById('fd-footer');
    if (footerEl) footerEl.outerHTML = window.fdFooter();
  });
})();
