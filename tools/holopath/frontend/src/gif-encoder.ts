...
export interface GifFrame {
  data: ImageData;
  delay: number; // ms (will be rounded to centiseconds)
}

export interface GifOptions {
  loop?: number;  // 0 = infinite loop (default)
}

// ─── Colour quantisation (median cut) ───────────────

interface ColorBox {
  pixels: Uint8Array[]; // each is [r, g, b]
  rMin: number; rMax: number;
  gMin: number; gMax: number;
  bMin: number; bMax: number;
}

function buildBox(pixels: Uint8Array[]): ColorBox {
...
function medianCutPalette(imageData: ImageData, maxColors: number = 256): Uint8Array[] {
  ...
  if (pixels.length === 0) {
    ...
    for (let i = 0; i < maxColors; i++) pal.push(new Uint8Array([0, 0, 0]));
    return pal;
  }

  // Median cut
  let boxes: ColorBox[] = [buildBox(pixels)];
...
function buildPaletteLookup(palette: Uint8Array[]): (r: number, g: number, b: number) => number {
...
function quantiseFrame(
  imageData: ImageData,
  _palette: Uint8Array[],
  findNearest: (r: number, g: number, b: number) => number,
): Uint8Array {
  const data = imageData.data;
...
function writeBytes(parts: (number | Uint8Array)[]): Uint8Array {
  let total = 0;
