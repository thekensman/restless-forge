...
function fitSubject(srcCanvas, maxSize) {
...
function drawRotated(target, src, x, y, degrees) {
...
// ─── Layout composers ───────────────────────────────
function composePyramid4(frame, outputSize) {
    ...
    ctx.clip();
    drawRotated(ctx, subject, Math.round(cx - fw / 2), outputSize - fh - inset, 180);
    ctx.restore();
...
function composePyramid3(frame, outputSize) {
    const [, ctx] = createCanvas(outputSize, outputSize);
    ctx.fillStyle = "#000";
    ctx.fillRect(0, 0, outputSize, outputSize);
...
function composeFan(frame, outputSize) {
    const [, ctx] = createCanvas(outputSize, outputSize);
    ctx.fillStyle = "#000";
    ctx.fillRect(0, 0, outputSize, outputSize);
    ...
    maskCtx.beginPath();
    maskCtx.arc(outputSize / 2, outputSize / 2, (outputSize - margin * 2) / 2, 0, Math.PI * 2);
    maskCtx.fill();
...
function composePeppersGhost(frame, outputSize) {
...
function composeSingle(frame, outputSize) {
    const srcCanvas = imageDataToCanvas(frame);
 ...
 */
export function composeLayout(frame, layout = "pyramid4", outputSize = 720) {
