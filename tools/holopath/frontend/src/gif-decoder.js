...
export function decodeGif(raw) {
    ...
    // ─── Global Colour Table ───
    const gct = hasGCT ? r.readBytes(gctSize * 3) : new Uint8Array(0);
    // ─── Parse frames ───
    const frames = [];
    ...
    while (r.hasMore()) {
        ...
        if (introducer === 0x2C) {
            ...
            const frameData = new ImageData(new Uint8ClampedArray(canvasData), width, height);
 ...
 */
export function isAnimatedGif(raw) {
    try {
