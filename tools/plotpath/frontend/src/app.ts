...
function handleChange(): void {
  const out = $("#output-area");
  if (out) out.textContent = "PlotPath ready — all processing runs in your browser.";
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  setupReactiveInputs();
  handleChange();
  console.log("PlotPath: loaded — browser-only, no backend");
});

// Expose engine for console debugging
(window as unknown as Record<string, unknown>)["plotpathEngine"] = engine;
