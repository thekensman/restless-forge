/* shared.js — ForgeImage shared header & footer
   Single source of truth for ALL ForgeImage pages (main app + sub-pages).
   Requires /shared.js (site/shared.js) to be loaded first for rf* utilities.

   Placeholders (replace across the whole template):
     forgeimage   — URL directory name,  e.g. "my-tool"
     ForgeImage  — Display name,        e.g. "My Tool"
     fimg — Identifier prefix,   e.g. "mt"  (2–5 lowercase letters)
     🖼️  — Header emoji/glyph,  e.g. "\u{1F527}"
   ...
   these two empty divs; this script auto-injects on DOMContentLoaded:
     <div id="fimg-header"></div>
     <div id="fimg-footer"></div>
*/
(function () {
  'use strict';
  ...
  window.fimgHeader = function () {
    ...
    return '<header class="site-header"><div class="site-header__inner">' +
      ...
      '<a class="site-header__brand" href="' + base + '/">🖼️ ForgeImage</a>' +
  ...
  window.fimgFooter = function () {
    ...
    return '<footer class="footer">' +
      ...
      '<p class="footer__copy">&copy; 2026 <a href="/" style="color:inherit;text-decoration:none;">Restless Forge</a> &mdash; ForgeImage.</p>' +
      '</footer>';
  };

  document.addEventListener('DOMContentLoaded', function () {
    var headerEl = document.getElementById('fimg-header');
    if (headerEl) headerEl.outerHTML = window.fimgHeader();
    var footerEl = document.getElementById('fimg-footer');
    if (footerEl) footerEl.outerHTML = window.fimgFooter();
  });
})();
